...
class Phabricator(object):
    ...
    def set_diff_properties(self, diff, publicbase, parent):
        params = {
            'diff_id': diff['id'],
            'name': 'mozilla:meta',
            'data': json.dumps({
        ...
        response = self._request(
            'GET', 'differential.setdiffproperty', params
        ).json()
        ConduitError.raise_if_error(response)

    def create_revision(self, diff, revision=None):
